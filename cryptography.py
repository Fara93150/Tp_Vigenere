...
def vigenere_cipher(text, password):
    list_of_keys = [ord(char) for char in password]# ord=le numéro de la lettre ascii
    crypted_text = []
    for index, char in enumerate(text):
        current_key = list_of_keys[index %len(list_of_keys)]
        crypted_text.append(cesar_cipher(char,current_key))#append ajouter un élémént dans la liste
